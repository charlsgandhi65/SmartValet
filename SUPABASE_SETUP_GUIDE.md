# Complete Supabase Setup Guide for SmartValet

This guide contains everything you need to set up your Supabase database for the SmartValet application.

## 📋 Table of Contents
1. [Step 1: Enable Required Extensions](#step-1-enable-required-extensions)
2. [Step 2: Create All Tables](#step-2-create-all-tables)
3. [Step 3: Create Indexes](#step-3-create-indexes)
4. [Step 4: Enable Row Level Security](#step-4-enable-row-level-security)
5. [Step 5: Create RLS Policies](#step-5-create-rls-policies)
6. [Step 6: Create Functions & Triggers](#step-6-create-functions--triggers)
7. [Step 7: Insert Sample Data (Optional)](#step-7-insert-sample-data-optional)
8. [Step 8: Verify Setup](#step-8-verify-setup)

---

## Step 1: Enable Required Extensions

Run this in Supabase SQL Editor to enable UUID generation:
...
**⚠️ IMPORTANT: Run these in order as some tables reference others.**

```sql
-- ============================================
-- 1. CUSTOMER TABLE (MISSING FROM ORIGINAL SCHEMA)
-- ============================================
...
**This is the most important part!** These policies control who can read/write data.
...
-- Drop existing policies if they exist (optional - helps if re-running script)
...
-- Allow anyone to read customer data (for login verification)
...
-- Only allow authenticated admins to insert (you may want to restrict this)
...
-- Allow public read (anyone can view events)
CREATE POLICY "Allow public select on events"
ON public.events FOR SELECT
TO public
USING (true);

-- Allow public insert (admins create events via app)
CREATE POLICY "Allow public insert on events"
ON public.events FOR INSERT
TO public
WITH CHECK (true);

-- Allow public update (admins edit events)
...
-- Allow public delete (admins delete events)
...
-- Allow public insert (admins create staff)
CREATE POLICY "Allow public insert on staff"
ON public.staff FOR INSERT
TO public
WITH CHECK (true);

-- Allow public update (update staff info)
...
-- Allow public delete (admins delete staff)
...
-- Allow public delete (cancel bookings)
...
-- Allow public insert (staff log QR scans)
...
-- Allow public insert (customers create requests)
CREATE POLICY "Allow public insert on parking_requests"
ON public.parking_requests FOR INSERT
TO public
WITH CHECK (true);

-- Allow public update (staff accept/complete requests)
...
These automatically update the `updated_at` timestamp:
...
-- Parking requests table trigger
...
**Current Setup:** The policies above allow public access (anyone with the API key) to read/write data. This is suitable for:

- ✅ Development/Testing
- ✅ Mobile apps using anonymous API keys
- ✅ Apps where Supabase Auth handles authentication

**For Production**, consider:

1. **Restrict RLS policies** based on authenticated users
2. **Use Service Role key** only on backend servers
3. **Implement proper authentication** checks in policies
4. **Add additional policies** that check user roles

Example production policy (restrict customers to their own data):
...
TO public
USING (
    -- Allow if it's the customer's own booking
    customer_email = current_setting('request.jwt.claims', true)::json->>'email'
    OR 
    -- Allow staff/admins to see all (you'd need to add role checks)
    true
);
```
...
```
customer (1) ────< (many) booked_parking
events (1) ────< (many) booked_parking
events (1) ────< (many) staff
staff (1) ────< (many) qr_logs
booked_parking (1) ────< (many) qr_logs
booked_parking (1) ────< (many) parking_requests
staff (1) ────< (many) parking_requests
```
...
1. **Run all SQL scripts** in Supabase SQL Editor (in order)
2. **Test your Android app** - try signing up a customer
3. **Create a test admin** account via SQL or Supabase dashboard
4. **Test the full flow**: Signup → Login → Book Parking → View Bookings
...
- Solution: Make sure all RLS policies are created correctly
- Check if policies allow the operation you're trying to perform

**Issue: "Table does not exist"**
- Solution: Make sure you ran Step 2 (Create All Tables) in order
- Customer table must be created before booked_parking table

**Issue: "Foreign key violation"**
- Solution: Make sure referenced data exists (e.g., customer email, event_id)

**Issue: "Column does not exist"**
- Solution: Check table schema matches what your app expects
...
2. Verify your API keys are correct in `SupabaseClientInstance.java`
